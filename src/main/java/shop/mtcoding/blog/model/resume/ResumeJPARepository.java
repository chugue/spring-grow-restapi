...
//    @Query("select r from Resume r join fetch Resume r on a.resume.id = r.id where a.isPass not in ('1') and a.resume.id = :resumeId")
//    Apply findAllSessionUserState(@Param("resumeId")Integer resumeId);

    @Query("select r from Resume r join fetch Apply a on a.resume.id = r.id join fetch User u on r.user.id = u.id where a.isPass not in ('1') and r.user.id = :userId")
